document.addEventListener("DOMContentLoaded", () => {
    ...
    if (typeof businessOwnerId !== "undefined") {
        socket = new WebSocket('ws://' + window.location.host + '/ws/chat/' + businessOwnerId + '/');

        socket.onmessage = function (e) {
            const data = JSON.parse(e.data);
            const messageElement = document.createElement("div");
            messageElement.classList.add("message", data.sender === userId ? "sent" : "received");
            messageElement.innerHTML = `<p>${data.message}</p><small>${data.timestamp}</small>`;
